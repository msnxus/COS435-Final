...
class PolicyNetwork(nn.Module):
  def __init__(self, n_V, n_T, hidden_dim=32):
      self.V = n_V
      self.T = n_T
      super(PolicyNetwork, self).__init__()
      self.f1 = nn.Linear(self.V, hidden_dim)
      self.f2 = nn.Linear(hidden_dim, hidden_dim)
      self.f3 = nn.Linear(hidden_dim, self.V)
...
def train(num_steps=1000, mini_batch_size=8, ppo_epochs=4, threshold=400):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    env = TrafficEnv(trafficmap_dir='traffic_maps', time_per_step=10, max_steps=8)
