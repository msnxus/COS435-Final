...
class sb3_CityEnv(gym.Env):
  def __init__(self, poly_matrix, N, time_horizon, max_steps=5000):
      super().__init__()
  ...
  def _get_obs(self):
      ...
      _dests = self.destinations.astype(np.float32)
      _current = np.array([self.current_vertex], dtype=np.float32)
      return np.concatenate([_time, _dests, _current], dtype=np.float32)
  ...
  def step(self, action):
      ...
      self.current_vertex = j # Move to current vertex
      self.num_steps += 1
      reward -= travel_time
